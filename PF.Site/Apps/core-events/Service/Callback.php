...
class Callback extends \Phpfox_Service
{
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => array('full_name_commented_on_a_comment_posted_on_the_event_title',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aRow['title'])),
                'owner_message' => array('full_name_commented_on_one_of_your_comments_you_posted_on_the_event', array(
                ...
                'mass_id' => 'event',
                'mass_subject' => (Phpfox::getUserId() == $aRow['user_id'] ? array('full_name_commented_on_one_of_gender_event_comments',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1)
                    )) : array('full_name_commented_on_one_of_row_full_name_s_event_comments',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'row_full_name' => $aRow['full_name']))),
                'mass_message' => (Phpfox::getUserId() == $aRow['user_id'] ? array('full_name_commented_on_one_of_gender_own_comments_on_the_event',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1),
                        'item_link' => $sItemLink,
                        'title' => $aRow['title'],
                        'link' => $sLink
                    )) : array('full_name_commented_on_one_of_row_full_name_s', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
    ...
    public function addItemNotification($aParams)
    {
        ...
        if ($aParams['item_type'] == 'photo' && Phpfox::isModule('photo')) {
            ...
            $sSubject = array('full_name_added_photo_s_on_your_event_title',
                array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'title' => $aEvent['title']
                ));
            $sMessage = array('full_name_added_photo_s_on_your_event_message',
                array(
                    'full_name' => Phpfox::getUserBy('full_name'),
