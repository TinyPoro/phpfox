<div class="quizzes-app feed {if empty($aQuiz.image_path)}no-photo{/if}">
	<div class="quizzes-media">
        ...
        {/if}
		<span class="quizzes-vote-number {if $aQuiz.total_play > 99}more{/if}"><b>{$aQuiz.total_play|short_number}</b>{if $aQuiz.total_play == 1}{_p('quiz_total_play')}{else}{_p('quiz_total_plays')}{/if}</span>
	</div>
	<div class="quizzes-inner pl-2 pr-2">
		<a href="{if $bIsFeedSponsor && $iSponsorId}{url link='ad.sponsor' view=$iSponsorId}{else}{permalink title=$aQuiz.title id=$aQuiz.quiz_id module='quiz'}{/if}" class="quizzes-title fw-bold">{$aQuiz.title|clean}</a>
