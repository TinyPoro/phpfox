...
class Process extends \Phpfox_Service
{
    ...
    public function approveQuiz($iQuiz)
    {
        ...
        if (Phpfox::isModule('feed') && Phpfox::getParam('quiz.quiz_allow_create_feed_when_add_new_item', 1)) {
            Phpfox::getService('feed.process')->add('quiz', $aQuiz['quiz_id'],
                $aQuiz['privacy'], (isset($aQuiz['privacy_comment']) ? (int)$aQuiz['privacy_comment'] : 0), 0, $aQuiz['user_id']);
        }

        // Send the user an email
        $sLink = Phpfox_Url::instance()->permalink('quiz', $aQuiz['quiz_id'], $aQuiz['title']);
        Phpfox::getLib('mail')->to($aQuiz['user_id'])
            ->subject(array('your_quiz_title_has_been_approved', array('title' => $aQuiz['title'])))
            ->message(array('your_quiz_a_href_link_title_a_has_been_approved_to_view_this_quiz_follow_the_link_below_a_href_link_link_a',
                array('link' => $sLink, 'title' => $aQuiz['title'])))
            ->send();
    ...
    public function update($aQuiz, $iUser)
    {
        ...
        if (isset($aQuiz['q']) && (Phpfox::getUserParam('quiz.can_edit_others_questions') ||
    ...
    {
        $aQuiz = Phpfox::getService('quiz')->getQuizById((int)$iQuiz);
        ...
        if(!empty($aQuiz) && ((int)$aQuiz['view_id'] == 0) )
        {
            // Update user activity
        ...
        (Phpfox::isAppActive('Core_BetterAds') ? Phpfox::getService('ad.process')->closeSponsorItem('quiz',
            (int)$iQuiz) : null);
    ...
    public function add(&$aVals, $iUser)
    {
        ...
        if (!Phpfox::getUserParam('quiz.new_quizzes_need_moderation')) {
            if (Phpfox::isModule('feed') && Phpfox::getParam('quiz.quiz_allow_create_feed_when_add_new_item', 1)) {
                Phpfox::getService('feed.process')->add('quiz', $iQuizId, $aVals['privacy'],
