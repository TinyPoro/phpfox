...
class Callback extends \Phpfox_Service
{
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => array('full_name_commented_on_one_of_your_quiz_title',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aQuiz['title'], 'link' => $sLink)),
                'owner_message' => array('full_name_commented_on_your_quiz_a_href',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'title' => $aQuiz['title'])),
                'owner_notification' => 'comment.add_new_comment',
                'notify_id' => 'comment_quiz',
                'mass_id' => 'quiz',
                'mass_subject' => (Phpfox::getUserId() == $aQuiz['user_id'] ? array('full_name_commented_on_gender_quiz',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aQuiz['gender'], 1)
                    ))
                    : array('full_name_commented_on_other',
                        array('full_name' => Phpfox::getUserBy('full_name'), 'other_full_name' => $aQuiz['full_name'])))
            ,
                'mass_message' => (Phpfox::getUserId() == $aQuiz['user_id'] ?
                    array('full_name_commented_on_gender_quiz', array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aQuiz['gender'], 1),
                        'title' => $aQuiz['title'],
                        'link' => $sLink
                    ))
                    : array('full_name_commented_on_other_full_name_s_quiz', array(
                        'full_name' => Phpfox::getUserBy('full_name'),
    ...
    public function getToSponsorInfo($iId)
    {
        ...
        $aQuiz['title'] = _p('quiz_sponsor_item_title', array('sQuizTitle' => $aQuiz['title']));
        $aQuiz['paypal_msg'] = _p('quiz_sponsor_paypal_message_title', array('sQuizTitle' => $aQuiz['title']));
