...
class Process extends \Phpfox_Service
{
    ...
    public function publicSong($iSongId)
    {
        if(!$iSongId)
        {
            return false;
        ...
        $bNewFeed = isset($_SESSION['music_song_'.$aSong['time_stamp'].'_'.$aSong['album_id']]) && $_SESSION['music_song_'.$aSong['time_stamp'].'_'.$aSong['album_id']] > 0;
        if ($bNewFeed) {
            db()->insert(Phpfox::getT('music_feed'), array(
                    'feed_id' => $_SESSION['music_song_'.$aSong['time_stamp'].'_'.$aSong['album_id']],
        ...
        if ((int)$aSong['album_id'] > 0) {
            if (!Phpfox::getUserParam('music.music_song_approval')) {
                ...
                if (!$bNewFeed) {
                    (Phpfox::isModule('feed') && Phpfox::getParam('music.music_allow_create_feed_when_add_new_item', 1) ? $_SESSION['music_song_'.$aSong['time_stamp'].'_'.$aSong['album_id']] = $iFeedId = \Phpfox::getService('feed.process')->callback($aCallback)->add('music_song',
