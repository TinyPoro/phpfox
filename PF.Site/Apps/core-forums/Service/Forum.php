...
class Forum extends Phpfox_Service
{
    ...
    private function _buildJump(
        ...
        foreach ($this->_aForums as $aForum) {
            ...
            $sOptions .= '<option value="' . ($bIdOnly ? $aForum['forum_id'] : Phpfox_Url::instance()->permalink('forum',
                    $aForum['forum_id'],
                    _p($aForum['name']))) . '"' . (($bSelectAll || $this->_iActive == $aForum['forum_id'] || in_array($aForum['forum_id'],
                        $aValues)) ? ' selected="selected"' : '') . '>' . $sExt . Phpfox_Locale::instance()->convert(_p($aForum['name'])) . '</option>' . "\n";
    ...
    private function _buildAdminCp($iForumId)
    {
        ...
        foreach ($this->_aForums as $aForum) {
            ...
            $sHtml .= '<li><i class="fa fa-sort" style="padding-right: 10px;"></i> <input type="hidden" name="order[' . $aForum['forum_id'] . ']" value="' . $iCnt . '" /> <a href="#?id=' . $aForum['forum_id'] . '" class="js_drop_down">' . Phpfox_Locale::instance()->convert(_p($aForum['name'])) . '</a>';
    ...
    public function getSearchFilter($bIsSearchQuery = false, $forumId = 0, $aParam = array(), $sView = '')
    {
        ...
        $sortDefault = 'ft.time_stamp';
        if($sView == 'new' || $sView == 'pending-post') {
            unset($aSorts['ft.total_post']);
            unset($aSorts['ft.total_view']);
            $sortDefault = 'fp.time_stamp';
        }
        ...
        $defaultValue = _p('search_discussions');
        $searchFields = array('ft.title');
        switch ($sView) {
            case 'new':
            case 'pending-post':
                $defaultValue = _p('search_posts');
                $searchFields = array('ft.title', 'fp.title', 'fpt.text');
                break;
            case 'subscribed':
            case 'pending-thread':
            case 'my-thread':
                $defaultValue = _p('search_threads');
                $sortData[] = array(
                    'link' => 'total_post',
    ...
    public function getForRss($iId)
    {
        ...
        $aRss = array(
            ...
            'title' => _p('latest_threads_in') . ': ' . _p($aForum['name']),
