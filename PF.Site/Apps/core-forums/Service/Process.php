...
class Process extends Core_Service_Systems_Category_Process
{
    ...
    public function addSponsorForFree($aVals)
    {
        $sSection = !empty($aVals['section']) ? $aVals['section'] : '';

        if (empty($sSection)) {
            $iAutoPublish = Phpfox::getUserParam('forum.auto_publish_sponsored_item');
        } else {
            $iAutoPublish = Phpfox::getUserParam('forum.auto_publish_sponsored_' . $sSection);
        }
        ...
        array_unshift($aGenders, 0);


        $aInsertSponsor = array(
            'module_id' => 'forum' . (!empty($sSection) ? '_' . $sSection : ''),
        ...
        Phpfox::callback('forum.enableSponsor' . ($sSection ? ucfirst($sSection) : ''),
            ['item_id' => $aVals['item_id']]);
    ...
    {
        foreach ($aOrders as $iForumId => $iOrder) {
            $this->database()->update($this->_sTable, ['ordering' => $iOrder], 'forum_id = ' . (int)$iForumId);
        }
