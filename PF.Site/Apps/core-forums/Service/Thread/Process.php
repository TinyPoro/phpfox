...
class Process extends Phpfox_Service
{
    ...
    public function update($iId, $iUserId, $aVals)
    {
        ...
        {
            if(Phpfox::getUserParam('forum.can_sponsor_thread') && $aUpdate['order_id'] == 2)
            {

                $iCnt = db()->select('COUNT(*)')
                    ->from(Phpfox::getT('better_ads_sponsor'))
                    ->where('module_id = "forum_thread" AND item_id = '.(int)$iId. ' AND user_id = '.(int)$iUserId)
                    ->execute('getSlaveField');
                if(!$iCnt)
                {
                    Phpfox::getService('forum.process')->addSponsorForFree(
                        array(
                            'section' => 'thread',
                            'item_id' => $iId,
                            'name' => _p('default_campaign_custom_name',
                                ['module' => _p('forum_thread'), 'name' => $aUpdate['title']])
                        ));
                }
                else
                {
                    db()->update(Phpfox::getT('better_ads_sponsor'),['campaign_name' => _p('default_campaign_custom_name',
                        ['module' => _p('forum_thread'), 'name' => $aUpdate['title']])], 'module_id = "forum_thread" AND item_id = '.(int)$iId. ' AND user_id = '.(int)$iUserId);
                }
            }
            else
            {
                db()->delete(Phpfox::getT('better_ads_sponsor'),'module_id = "forum_thread" AND item_id = '.(int)$iId. ' AND user_id = '.(int)$iUserId);
            }
            $this->cache()->remove('forum_thread_sponsored');
        }
    ...
    public function add($aVals, $aCallback = false, $aExtra = array())
    {
        ...
        if(Phpfox::isAppActive('Core_BetterAds') && Phpfox::getUserParam('forum.can_sponsor_thread') && $aInsert['order_id'] == 2)
        {
            Phpfox::getService('forum.process')->addSponsorForFree(
                array(
                    'section' => 'thread',
                    'item_id' => $iId,
                    'name' => _p('default_campaign_custom_name',
                        ['module' => _p('forum_thread'), 'name' => $aInsert['title']])
                ));
            $this->cache()->remove('forum_thread_sponsored');
        }
        ...
        if (Phpfox::isModule('tag') && Phpfox::getParam('tag.enable_tag_support')) {
            if (isset($aVals['tag_list']) && ((is_array($aVals['tag_list']) && count($aVals['tag_list'])) || (!empty($aVals['tag_list'])))) {
                ...
                $this->cache()->remove('tag_cloud_forum');
            }
        }

        if ($iLoop === 1 && empty($aExtra) && !isset($bSkipFeedEntry)) {
            ((Phpfox::isModule('feed') && !defined('PHPFOX_SKIP_FEED_ENTRY')  && Phpfox::getParam('forum.forum_allow_create_feed_when_add_new_item', 1)) ? Phpfox::getService('feed.process')->callback($aCallback)->add('forum',
        ...
        $this->cache()->remove('forum_start_posts_ids');
    ...
    {
        $aThread = $this->database()->select('ft.forum_id, ft.group_id, ft.thread_id, ft.user_id, ft.poll_id, ft.view_id')
            ->from($this->_sTable, 'ft')
            ->where('ft.thread_id = ' . (int)$iThread)
            ->execute('getSlaveRow');
        ...
        $aPosts = $this->database()->select('post_id, user_id, view_id')
            ->from(Phpfox::getT('forum_post'))
        ...
        (Phpfox::isAppActive('Core_BetterAds') ? Phpfox::getService('ad.process')->closeSponsorItem('forum_thread',
            (int)$iThread) : null);
        (($sPlugin = Phpfox_Plugin::get('forum.service_process_delete__1')) ? eval($sPlugin) : false);

        if(Phpfox::isModule('tag'))
        {
            Phpfox::getService('tag.process')->deleteForItem($aThread['user_id'], $iThread, 'forum');
            $this->cache()->remove('tag_cloud_forum');
        }

        if((int)$aThread['view_id'] == 0)
        {
            Phpfox::getService('user.activity')->update($aPost['user_id'], 'forum', '-');
        }
        if ($aThread['is_sponsor'] == 1) {
            $this->cache()->remove('forum_thread_sponsored');
        }
