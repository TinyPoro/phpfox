	...
	<div class="item-outer pt-2 pb-2">
		<div class="item-inner">
			<div class="item-title">
					<h5 itemprop="name">
						<a href="{permalink module='forum' id=$aForum.forum_id title=$aForum.name}"{if !empty($aForum.description)} title="{_p var=$aForum.description}" {/if} class="forum-text-overflow fw-bold" itemprop="url">{if $aForum.is_category}<i class="ico mr-1 text-transition item-category"></i>{else}<i class="ico mr-1 item-forum"></i>{/if}{_p var=$aForum.name}</a>
                    </h5>
					{if !empty($aForum.description)}
						<h6 class="item-description text-transition">
                            {_p var=$aForum.description}
                        </h6>
					{/if}
			</div>
