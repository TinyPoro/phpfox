...
class Ajax extends Phpfox_Ajax
{
    public function addReply()
    {
        ...
        if (($iFlood = Phpfox::getUserParam('forum.forum_post_flood_control')) !== 0) {
            ...
            if (Phpfox::getLib('spam')->check($aFlood)) {
                $this->alert(_p('posting_a_new_reply_a_little_too_soon') . ' ' . Phpfox::getLib('spam')->getWaitTime());
                $this->call('$(\'#js_forum_reply_submit_btn, #js_forum_form_submit_btn\').removeClass(\'disabled\');');
                return false;
            }
        }
        ...
        if ($iId = Phpfox::getService('forum.post.process')->add($aVals, $aCallback)) {
            if(!empty($aVals['perma_view']))
            {
                $sThreadUrl = Phpfox_Url::instance()->makeUrl('forum.thread.'.$aVals['thread_id']);
                $this->call('window.location.href = "'.$sThreadUrl.'";');
                return null;
            }
    ...
    public function deletePost()
    {
        ...
        if ((Phpfox::getService('forum.moderate')->hasAccess($aPost['forum_id'], 'delete_post') ||
            Phpfox::getService('user.auth')->hasAccess('forum_post', 'post_id', $this->get('id'), 'forum.can_delete_own_post', 'forum.can_delete_other_posts'))
        ) {
            $bHasAccess = true;
        }
    ...
    public function sponsor()
    {
        ...
        if (Phpfox::getService('forum.thread.process')->sponsor($iThreadId, $iType)) {
            ...
            if ($iType == '2') {
                ...
                {
                    $sHtml = '<a href="'.Phpfox_Url::instance()->makeUrl('ad.sponsor.'.$iThreadId.'.section_forum_thread').'"><i class="ico ico-sponsor mr-1"></i>'._p('sponsor').'</a>';
                    $this->html('#js_sponsor_purchase_thread_'.$iThreadId, $sHtml);
                }
