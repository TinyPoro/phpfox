...
class AddCompareController extends Phpfox_Component
{
    public function process()
    {
        ...
        if($bIsEdit)
        {
            ...
            foreach($aPackages as $iKey => $aPackage)
            {
                foreach($aFeatureForPackages as $packageid => $feature)
                {
                    ...
                    {
                        $sText  = _p($feature['text']);
                        $aTexts[$feature['text']] = $sText;
                    }
        ...
        $this->template()->setTitle(($bIsEdit) ? _p('Edit Compare Feature').': '._p($aFeature['feature_title']) : _p('Add Compare Feature'))
            ->setBreadCrumb(_p('subscriptions'),$this->url()->makeUrl('admincp.subscribe'))
            ->setBreadCrumb(($bIsEdit ? _p('Edit Compare Feature') : _p('Add Compare Feature')), null, true)
