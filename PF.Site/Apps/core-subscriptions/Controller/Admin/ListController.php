    ...
    public function process()
    {
        ...
        $aFilters = array(
            'status' => array(
                ...
                'add_any' => true,
                'search' => ($aSearch['status'] == '' ? 'AND (sp.status IS NOT NULL OR sp.status != \'\')' : 'AND sp.status = \'[VALUE]\''),
                'id' => 'status'
            ),
            'display' => array(
                'type' => 'select',
            ...
            'period' => array(
                ...
                'search' => ($aSearch['period'] == 'custom') ? ' AND (sp.time_stamp BETWEEN '.strtotime($aSearch['from'].' 00:00:00').' AND '.strtotime($aSearch['to'].' 23:59:59').')'.(empty($aSearch['status']) ? ' AND (sp.status IS NOT NULL OR sp.status != \'\')' : '') : ''
            ),
            'reason' => array(
                'type' => 'select',
                'id' => 'reason',
