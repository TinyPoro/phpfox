...
$Behavior.core_subscriptions_admincp = function () {
    var default_language = $('#default-language-id').val();
    $('#title_' + default_language).on('input',function () {
        $('#character-number-title').html($(this).val().length);
    });
    $('#description_' + default_language).on('input',function () {
        $('#character-number-description').html($(this).val().length);
    });
...
$Behavior.add_compare = function () {
    ...
    $('.core-subscriptions-admincp-add-compare textarea').on('input', function () {
        var id = $(this).parent().parent().data('id');
        $('#content-number-' + id).html($(this).val().length);
    });
    $('.core-subscriptions-admincp-add-compare #feature_title_' + default_language).on('input', function () {
        $('#character-number-title').html($(this).val().length);
    });
};
