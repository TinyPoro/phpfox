...
class Process extends Phpfox_Service
{
    ...
    public function add($aVals, $iUpdateId = null)
    {
        ...
        {
            $aInputCosts = $aVals['cost'];
            foreach($aInputCosts as $keycost => $aInputCost)
            {
                $aInputCosts[$keycost] = 0;
            }
            $aVals['cost'] = serialize($aInputCosts);
        }
        else
        {
    ...
    public function delete($iId)
    {
        ...
        if($bHasSubscription)
        {
            ...
            if((int)$aPackage['recurring_period'] > 0)
            {
                ...
                {
                    Phpfox::getLib('mail')
                        ->to($aActiveSubscription['email'])
                        ->subject(['subject_delete_package_template',[
                            'package_title' => _p($aActiveSubscription['title'], [], !empty($aActiveSubscription['language_id']) ? $aActiveSubscription['language_id'] : $sDefaultLanguage)
                        ]])
                        ->message(['delete_package_template',[
                            'username' => $aActiveSubscription['full_name'],
                            'package_title' => _p($aActiveSubscription['title'], [], !empty($aActiveSubscription['language_id']) ? $aActiveSubscription['language_id'] : $sDefaultLanguage),
                            'expiry_date' => Phpfox::getLib('date')->convertTime($aActiveSubscription['expiry_date']),
                            'link' => \Phpfox_Url::instance()->makeUrl('subscribe')
                        ]])
                        ->send();
                    Phpfox::getService('notification.process')->add('subscribe_deletepackage', $aActiveSubscription['last_purchase_id'], $aActiveSubscription['user_id']);
                }
            }
