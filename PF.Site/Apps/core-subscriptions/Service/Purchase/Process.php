...
class Process extends Phpfox_Service
{
    ...
    public function updateOrder($aVals)
    {
        ...
        foreach ($aVals['ordering'] as $iId => $iOrder) {
            $this->database()->update(Phpfox::getT('subscribe_reason'), array('ordering' => (int)$iOrder), 'reason_id = ' . (int)$iId);
        }
        return null;
    }
    ...
    */
    ...
    public function notifyUserWithManualMethodPayment()
    {
        ...
        if (count($aSubscriptionPackages)) {
            ...
            foreach ($aSubscriptionPackages as $aSubscriptionPackage) {
                ...
                $iNotify = $aSubscriptionPackage['number_day_notify_before_expiration'];

                //Get user_id is expired
                $aListsExpired = db()->select('sp.user_id, sp.status, sp.purchase_id AS purchase_id, sp.expiry_date, u.full_name, u.language_id')
                    ->from(Phpfox::getT('subscribe_purchase'),'sp')
                    ->join(Phpfox::getT('user'),'u', 'u.user_id = sp.user_id')
                    ->where('sp.package_id=' . (int)$aSubscriptionPackage['package_id'] . ' AND (sp.status="completed") AND sp.renew_type = 2')
                    ->group('sp.user_id')
                    ->executeRows();

                foreach ($aListsExpired as $aListExpired) {
                    if(date('m/d/Y', (int)$aListExpired['expiry_date'] - ($iNotify * 24 *3600)) == date('m/d/Y', PHPFOX_TIME))
                    {
    ...
    public function updatePurchase($iPurchaseId, $sFromStatus, $sToStatus)
    {
        ...
        switch ($sFromStatus)
        {
            ...
            case 'pending':
                if($sToStatus == 'completed')
                {
                    ...
                    $this->database()->update(Phpfox::getT('user_field'), array('subscribe_id' => '0'), 'user_id = ' . (int) $aPurchase['user_id']);
                    $aCurrentActiveSubscriptions = db()->select('sp.*')
                                        ->from($this->_sTable,'sp')
        ...
        $this->database()->update($this->_sTable, array('status' => $sToStatus, 'time_stamp' => PHPFOX_TIME), 'purchase_id = ' . (int) $iPurchaseId);
