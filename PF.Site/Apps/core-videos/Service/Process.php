...
class Process extends Phpfox_Service
{
    ...
    public function addVideo($aVals)
    {
        ...
        if (Phpfox::isModule('feed') && !defined('PHPFOX_SKIP_FEED_ENTRY') && (user('pf_video_approve_before_publicly') == false) && Phpfox::getParam('v.pf_video_allow_create_feed_when_add_new_item', 1)) {
        ...
        if (isset($aVals['status_info'])) {
            ...
            foreach ($mentions as $user) {
                Phpfox::getLib('mail')->to($user->id)
                    ->subject(['user_name_tagged_you_in_video_tittle', [
                        'user_name' => $sTagger,
                        'title' => $aSql['title'],
                    ]])
