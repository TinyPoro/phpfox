...
class Process extends Phpfox_Service
{
    ...
    public function add($aVals)
    {
        ...
        if ($aVals['post_status'] == BLOG_STATUS_PUBLIC) {
            if (isset($aVals['module_id']) && ($aVals['module_id'] != 'blog') && Phpfox::isModule($aVals['module_id']) && Phpfox::hasCallback($aVals['module_id'],
                    'getFeedDetails')) {
                (Phpfox::isModule('feed') && Phpfox::getParam('blog.allow_create_feed_when_add_new_item', 1) ? Phpfox::getService('feed.process')->callback(Phpfox::callback($aVals['module_id'] . '.getFeedDetails',
                    $aVals['item_id']))->add('blog', $iId, $aVals['privacy'],
                    (isset($aVals['privacy_comment']) ? (int)$aVals['privacy_comment'] : 0), $aVals['item_id']) : null);
            } else {
                (Phpfox::isModule('feed') && Phpfox::getParam('blog.allow_create_feed_when_add_new_item', 1) ? Phpfox::getService('feed.process')->add('blog', $iId, $aVals['privacy'],
    ...
    public function update($iId, $iUserId, $aVals, &$aRow = null)
    {
        ...
        if ($aRow['is_approved'] == 1 && (int)$aRow['post_status'] == BLOG_STATUS_DRAFT && (int)$aVals['post_status'] == BLOG_STATUS_PUBLIC) {
            if (isset($aRow['module_id']) && ($aRow['module_id'] != 'blog') && Phpfox::isModule($aRow['module_id']) && Phpfox::hasCallback($aRow['module_id'],
                    'getFeedDetails')) {
                (Phpfox::isModule('feed') && Phpfox::getParam('blog.allow_create_feed_when_add_new_item', 1) ? Phpfox::getService('feed.process')->callback(Phpfox::callback($aRow['module_id'] . '.getFeedDetails',
    ...
    public function delete($iId, $bForce = false)
    {
        ...
        (Phpfox::isAppActive('Core_BetterAds') ? Phpfox::getService('ad.process')->closeSponsorItem('blog', (int)$iId) : null);

        // Update user activity
        if ($aBlog['post_status'] == BLOG_STATUS_PUBLIC && $aBlog['is_approved'] == 1) {
    ...
    public function approve($iId)
    {
        ...
        if ($aBlog['post_status'] == BLOG_STATUS_PUBLIC) {
            if (isset($aBlog['module_id']) && ($aBlog['module_id'] != 'blog') && Phpfox::isModule($aBlog['module_id']) && Phpfox::hasCallback($aBlog['module_id'],
                    'getFeedDetails')) {
                (Phpfox::isModule('feed') && Phpfox::getParam('blog.allow_create_feed_when_add_new_item', 1) ? Phpfox::getService('feed.process')->callback(Phpfox::callback($aBlog['module_id'] . '.getFeedDetails',
                    $aBlog['item_id']))->add('blog', $iId, $aBlog['privacy'], $aBlog['privacy_comment'],
                    $aBlog['item_id'], $aBlog['user_id']) : null);
            } else {
                (Phpfox::isModule('feed') && Phpfox::getParam('blog.allow_create_feed_when_add_new_item', 1) ? Phpfox::getService('feed.process')->add('blog', $iId, $aBlog['privacy'],
