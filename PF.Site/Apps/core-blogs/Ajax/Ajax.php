...
class Ajax extends Phpfox_Ajax
{
    ...
    /**
     * Un-sponsor for purchased items
     * @return bool|null|string
     */
    public function unSponsor()
    {
        if (!Phpfox::isAppActive('Core_BetterAds')) {
            return $this->alert('your_request_is_invalid');
        }
        ...
        //Only allow owner can un-sponsor
        if ($aItem['user_id'] != Phpfox::getUserId()) {
            return $this->alert('your_request_is_invalid');
        }

        Phpfox::getLib('database')->update(':blog', array('is_sponsor' => 0), 'blog_id = ' . (int)$iBlogId);
        Phpfox::getLib('database')->delete(Phpfox::getT('better_ads_sponsor'),
            'module_id = "blog" AND item_id = ' . (int)$iBlogId);

        $this->getTemplateLinkAfterProcess($iBlogId);

        $this->alert(_p('blog_successfully_un_sponsored'), null, 300, 150, true);
    ...
    public function getTemplateLinkAfterProcess($iBlogId)
    {
        ...
        $this->call('$("#page_blog_view .sticky-sponsored-icon").remove();');
