...
class Callback extends \Phpfox_Service
{
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => array('full_name_commented_on_a_comment_posted_on_the_page_title',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aRow['title'])),
                'owner_message' => array('full_name_commented_on_one_of_your_comments', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'item_link' => $sItemLink,
                    'title' => $aRow['title'],
                    'link' => $sLink
                ...
                'mass_id' => 'pages',
                'mass_subject' => (Phpfox::getUserId() == $aRow['user_id'] ? array('full_name_commented_on_one_of_gender_page_comments',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1)
                    )) : array('full_name_commented_on_one_of_other_full_name_s_page_comments',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'other_full_name' => $aRow['full_name']))),
                'mass_message' => (Phpfox::getUserId() == $aRow['user_id'] ? array('full_name_comment_on_one_of_gender',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1),
                        'item_link' => $sItemLink,
                        'title' => $aRow['title'],
                        'link' => $sLink
                    )) : array('full_name_commented_on_one_of_other_full_name', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
    ...
    public function getFeedComment($iId, $aVals)
    {
        ...
        $aCallback = array(
            ...
            'subject' => array('full_name_wrote_a_comment_on_your_page_title',
                array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aPage['title'])),
            'message' => array('full_name_wrote_a_comment_link',
        ...
        if (empty($aPage)) {
            return array(
                'error' => _p('sorry_the_page_you_are_looking_for_no_longer_exists',
                    array('link' => Phpfox::getLib('url')->makeUrl('pages')))
            );
        }

        $aPage['link'] = Phpfox::permalink('pages', $aPage['item_id'], $aPage['title']);
        $aPage['paypal_msg'] = _p('sponsor_paypal_message_page', array('sPageTitle' => $aPage['title']));
        $aPage['image_dir'] = 'pages.url_image';
        $aPage['title'] = _p('sponsor_title_page', array('sPageTitle' => $aPage['title']));
    ...
    public function getNotificationReassign_Owner($aNotification)
    {
        ...
        $sPhrase = _p('full_name_just_assigned_you_as_owner_of_page_title', array(
    ...
    public function getNotificationOwner_Changed($aNotification)
    {
        ...
        $sPhrase = _p('full_name_just_transfer_your_page_title_to_other_user', array(
            'full_name' => Phpfox::getService('notification')->getUsers($aNotification),
            'title' => \Phpfox::getLib('parse.output')->shorten($aRow['title'],
                \Phpfox::getParam('notification.total_notification_title_length'), '...')
        ));
