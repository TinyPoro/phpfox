...
class Callback extends Phpfox_Service
{
    ...
    public function getToSponsorInfoAlbum($iId)
    {
        ...
        $aAlbum['title'] = _p('photo_album_sponsor_title', array('sAlbumTitle' => $aAlbum['name']));
        $aAlbum['paypal_msg'] = _p('album_sponsor_paypal_message', array('sAlbumTitle' => $aAlbum['name']));
        $aAlbum['link'] = \Phpfox_Url::instance()->makeUrl('photo.album.'.$aAlbum['item_id'].'.'.$aAlbum['name']);
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        $aSubject = (Phpfox::getUserId() == $aRow['user_id'] ?
            array('full_name_commented_on_gender_photo', array(
                'full_name' => Phpfox::getUserBy('full_name'),
                'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1)
            ))
            :
            array('full_name_commented_on_other_full_name_s_photo', array(
                'full_name' => Phpfox::getUserBy('full_name'),
                'other_full_name' => $aRow['full_name']
            )));
        $aMessage = (Phpfox::getUserId() == $aRow['user_id'] ?
            array('full_name_commented_on_gender_photo_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1),
                    'link' => $sLink,
            ...
            :
            array('full_name_commented_on_other_full_name_s_photo_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'other_full_name' => $aRow['full_name'],
                    'link' => $sLink,
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => array('full_name_commented_on_your_photo_title', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'title' => $this->preParse()->clean($aRow['title'], 100)
                )),
                'owner_message' =>
                    array('full_name_commented_on_your_photo_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'title' => $aRow['title'],
                            'link' => $sLink
        ...
        if ($aRow['type_id'] == 1 && $aRow['parent_user_id'] != 0 && $aRow['group_id'] == 0) {
            ...
            Phpfox::getService('comment.process')->notify(array(
                    ...
                    'item_id' => $aRow['photo_id'],
                    'owner_subject' => array('full_name_commented_on_your_photo_title',
                        array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aRow['title'])),
                    'owner_message' => array('full_name_commented_on_your_photo_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'link' => $sLink,
                            'title' => $aRow['title']
                    ...
                    'notify_id' => 'comment_photo',
                    'mass_id' => 'photo',
                    'mass_subject' => (Phpfox::getUserId() == $aRow['parent_user_id'] ? array('full_name_commented_on_gender_photo',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1)
                        )) : array('full_name_commented_on_other_full_name_s_photo',
                        array('full_name' => Phpfox::getUserBy('full_name'), 'other_full_name' => $aRow['full_name']))),
                    'mass_message' => (Phpfox::getUserId() == $aRow['parent_user_id'] ? array('full_name_commented_on_gender_photo_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1),
                            'link' => $sLink,
    ...
    public function getActivityFeed($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        $iSponsorId = Phpfox::isAppActive('Core_BetterAds') && ((int)$aItem['sponsor_feed_id'] === (int)$iFeedId) ? (int)(Phpfox::getService('ad.get')->getFeedSponsors($iFeedId)) : 0;

        $sLink = ($iSponsorId > 0 ? Phpfox::getLib('url')->makeUrl('ad.sponsor', ['view' => $iSponsorId]) : Phpfox::permalink('photo', $aRow['photo_id'], $aRow['title']) . ('feed_' . $iFeedId));
        $sFeedImageOnClick = '';
                ...
                if ($indexing == 2 && $total > 3) {
                    ...
                    if($photoMature) {
                        $aListPhotos[] = '<a href="#" class="no_ajax_link photo_holder_image photo-mature" onclick="tb_show(\'' . _p('warning') . '\', $.ajaxBox(\'photo.warning\', \'height=300&width=350&link=' . Phpfox::permalink('photo',
                                $aPhoto['photo_id'],
                                $aPhoto['title']) . ('feed_' . $iFeedId) . '/\')); return false;" style="background-image: url(\'' . $sPhotoImage . '\')"></a>';
                    }
                    else {
                        $aListPhotos[] = '<a href="' . Phpfox::permalink('photo', $aPhoto['photo_id'],
        ...
        if (empty($aListPhotos)) {
            ...
            $aListPhotos = [
                strtr('<a href="{href}" {other}><img src="{img_src}"></a>', [
                    '{href}' => $iSponsorId > 0 ? Phpfox::getLib('url')->makeUrl('ad.sponsor', ['view' => $iSponsorId]) : Phpfox::permalink('photo', $aRow['photo_id'], $aRow['title']) . ('feed_' . $iFeedId),
            ...
            $aListPhotos = array_merge(array(
                '<a href="' . Phpfox::permalink('photo', $aRow['photo_id'],
                    $aRow['title']) . ('feed_' . $iFeedId) . '/" ' . (empty($sFeedImageOnClick) ? ' class="' . $sThickbox . ' photo_holder_image" rel="' . $aRow['photo_id'] . '" ' : $sFeedImageOnClick . ' class="no_ajax_link photo_holder_image photo-mature"') . ' style="background-image: url(\'' . $sImageReturnUrl . '\')"></a>'
            ), $aListPhotos);
        }
    ...
    public function addCommentAlbum($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => array('full_name_commented_on_your_photo_album_name',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'name' => $aAlbum['name'])),
                'owner_message' =>
                    array('full_name_commented_on_your_photo_album_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'link' => $sLink,
                            'title' => $aAlbum['name']
                ...
                'mass_subject' => (Phpfox::getUserId() == $aAlbum['user_id'] ?
                    array('full_name_commented_on_gender_photo_album', array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aAlbum['gender'], 1)
                    ))
                    :
                    array('full_name_commented_on_other_full_name_s_photo_album', array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'other_full_name' => $aAlbum['full_name']
                    )))
            ,
                'mass_message' => (Phpfox::getUserId() == $aAlbum['user_id'] ?
                    array('full_name_commented_on_gender_photo_album_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'gender' => Phpfox::getService('user')->gender($aAlbum['gender'], 1),
                            'link' => $sLink,
                    ...
                    :
                    array('full_name_commented_on_other_full_name_s_photo_album_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'other_full_name' => $aAlbum['full_name'],
                            'link' => $sLink,
