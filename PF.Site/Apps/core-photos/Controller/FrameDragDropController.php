        ...
        if ($iId = $oServicePhotoProcess->add(Phpfox::getUserId(), array_merge($aVals, $aImage))) {
            ...
            if ($iWidth < $width) {
                ...
                if ($bIsCropped !== false) {
                    ...
                    $iNewImageSize = filesize(Phpfox::getParam('photo.dir_photo') . sprintf($sFileName, ''));
                    if(!empty($iNewImageSize))
                    {
                        Phpfox::getService('photo')->updatePhotoInfo($iId, ['file_size' => $iNewImageSize]);
                    }
                }
            }
            //End Crop
