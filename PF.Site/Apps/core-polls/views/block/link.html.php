...
{if $aPoll.canSponsor}
    <li>
        <a href="javascript:void(0);" id="js_sponsor_{$aPoll.poll_id}" onclick="$('#js_sponsor_phrase_{$aPoll.poll_id}').hide(); $('#js_sponsor_{$aPoll.poll_id}').hide();$('#js_unsponsor_{$aPoll.poll_id}').show();$.ajaxCall('poll.sponsor','poll_id={$aPoll.poll_id}&type=0', 'GET'); return false;" style="{if $aPoll.is_sponsor != 1}display:none;{/if}">
            <i class="ico ico-sponsor mr-1"></i>{_p var='unsponsor_this_poll'}
        </a>
        <a href="javascript:void(0);" id="js_unsponsor_{$aPoll.poll_id}" onclick="$('#js_sponsor_phrase_{$aPoll.poll_id}').show(); $('#js_sponsor_{$aPoll.poll_id}').show();$('#js_unsponsor_{$aPoll.poll_id}').hide();$.ajaxCall('poll.sponsor','poll_id={$aPoll.poll_id}&type=1', 'GET'); return false;" style="{if $aPoll.is_sponsor == 1}display:none;{/if}">
        <i class="ico ico-sponsor mr-1"></i>{_p var='sponsor_this_poll'}
        </a>
    </li>
{elseif $aPoll.canPurchaseSponsor}
    <li>
        {if (int)$aPoll.is_sponsor == 0}
