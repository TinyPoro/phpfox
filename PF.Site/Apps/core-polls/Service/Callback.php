...
class Callback extends \Phpfox_Service
{
    ...
    public function getActivityFeedComment($aRow)
    {
        ...
        $sTitle = Phpfox::getLib('parse.output')->shorten(Phpfox::getLib('parse.output')->clean($aItem['question']),
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => array('full_name_commented_on_one_of_your_polls_title',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aPoll['question'])),
                'owner_message' => array('full_name_commented_on_your_poll_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                    array(
                ...
                'mass_id' => 'poll',
                'mass_subject' => (Phpfox::getUserId() == $aPoll['user_id'] ? array('full_name_commented_on_gender_poll',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => \Phpfox::getService('user')->gender($aPoll['gender'], 1)
                    ))
                    : array('full_name_commented_on_other_full_name_s_poll', array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'other_full_name' => $aPoll['full_name']
                    ))),
                'mass_message' => (Phpfox::getUserId() == $aPoll['user_id'] ?
                    array('full_name_commented_on_gender_poll_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'gender' => \Phpfox::getService('user')->gender($aPoll['gender'], 1),
                            'link' => $sLink,
                    ...
                    :
                    array('full_name_commented_on_other_full_name_s_poll_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'other_full_name' => $aPoll['full_name'],
                            'link' => $sLink,
    ...
    public function getSearchInfo($aRow)
    {
        ...
        if (!empty($aRow['item_photo'])) {
            ...
            if ($aRow['item_time_stamp'] < \Apps\Core_Polls\Service\Poll::NO_THUMBNAIL_TIME) {
                unset($aItemPhoto['suffix']);
            }
            $aInfo['item_display_photo'] = Phpfox::getLib('image.helper')->display($aItemPhoto);
        }
