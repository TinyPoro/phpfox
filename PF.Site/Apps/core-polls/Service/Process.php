...
class Process extends \Phpfox_Service
{
    ...
    public function add($iUser, $aVals, $bIsUpdate = false)
    {
        ...
        if ($bIsUpdate) {
            ...
            if (!Phpfox::getUserParam('poll.poll_requires_admin_moderation') && !$bIsCustom) {
                (Phpfox::isModule('feed') && Phpfox::getParam('poll.poll_allow_create_feed_when_add_new_item', 1) ? \Phpfox::getService('feed.process')->add('poll', $iId, $aVals['privacy'],
                    (isset($aVals['privacy_comment']) ? (int)$aVals['privacy_comment'] : 0)) : null);
    ...
    public function moderatePoll($iPoll, $iResult)
    {
        ...
        if ($iResult == '0') {
            ...
            (Phpfox::isModule('feed') && Phpfox::getParam('poll.poll_allow_create_feed_when_add_new_item', 1) ? \Phpfox::getService('feed.process')->add('poll', $aPoll['poll_id'],
            ...
            Phpfox::getLib('mail')->to($aPoll['user_id'])
                ...
                ->message(array('your_poll_a_href_link_title_a_has_been_approved_to_view_this_poll_follow_the_link_below_a_href_link_link_a',
        ...
        if((int)$aPoll['view_id'] == 0 && empty($aPoll['module_id']))
        {
            \Phpfox::getService('user.activity')->update($aPoll['user_id'], 'poll', '-');
        }
        ...
        (Phpfox::isAppActive('Core_BetterAds') ? Phpfox::getService('ad.process')->closeSponsorItem('poll',
            (int)$iPoll) : null);
