    ...
    private function _getSponsors()
    {
        ...
        if ($aSearch = $this->request()->getArray('search')) {
            ...
            if (!empty($aSearch['from_month']) && !empty($aSearch['from_day']) && !empty($aSearch['from_year'])) {
                $iFromTimestamp = Phpfox::getService('ad.process')->convertToTimestamp($aSearch['from_day'], $aSearch['from_month'], $aSearch['from_year'], 'start');
            ...
            $iFromTimestamp = Phpfox::getService('ad.process')->convertToTimestamp($aForms['from_day'], $aForms['from_month'], $aForms['from_year'], 'start');
            $aCond[] = " AND s.start_date>=$iFromTimestamp";
