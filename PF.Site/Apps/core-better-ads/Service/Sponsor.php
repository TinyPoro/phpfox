...
class Sponsor extends \Phpfox_Service
{
    ...
    public function getAllSponsorSettings($iUserGroupId = 2, $sModuleId = '')
    {
        ...
        foreach (array_column($aItems, 'name') as $sSetting) {
            ...
            if ($sItem == '') {
                $sCanSponsorItem = "can_sponsor_$sModule";
                $sCanPurchaseSponsorItem = "can_purchase_sponsor";
                $sSponsorPrice = "{$sModule}_sponsor_price";
                $sAutoPublish = "auto_publish_sponsored_item";
            } else {
                $sCanSponsorItem = "can_sponsor_$sItem";
                $sCanPurchaseSponsorItem = "can_purchase_sponsor_$sItem";
                $sSponsorPrice = "{$sModule}_{$sItem}_sponsor_price";
                $sAutoPublish = "auto_publish_sponsored_$sItem";
            }

            $aSettings = array_merge($aSettings, [
                $sCanSponsorItem,
                $sCanPurchaseSponsorItem,
                $sSponsorPrice,
                $sAutoPublish,
            ]);
