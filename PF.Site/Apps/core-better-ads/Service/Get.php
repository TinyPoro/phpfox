...
class Get extends \Phpfox_Service
{
    ...
    public function getAdRedirect($iId)
    {
        ...
        if (($aAd['is_cpm'] != 1 && $aAd['count_click'] >= $aAd['total_click'])) {
            db()->update(':better_ads', array('is_active' => '0'), 'ads_id =' . $aAd['ads_id']);

            return Phpfox_Error::set(_p('better_ads_the_ad_you_are_looking_for_does_not_exist'));
        }
        db()->updateCounter('better_ads', 'count_click', 'ads_id', $aAd['ads_id']);
    ...
    public function getForEdit($iId)
    {
        ...
        if (!isset($aAd['gender'])) {
            $aAd['gender'] = array_unshift(array_keys(Phpfox::getService('core')->getGenders(true)), 0);
        } else {
    ...
    public function getSideBlockRecommendImageSizes()
    {
        return [
            // image type
            1 => '400 x 304 (' . _p('horizontal_image') . ') ' . _p('or') . ' 400 x 608 (' . _p('vertical_image') . ')',
            // html type
            2 => '400 x 304'
        ];
    }
