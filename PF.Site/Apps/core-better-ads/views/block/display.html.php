	        ...
	        <div id="ads_item_{$aAd.ads_id}" class="bts-block__item multi_ad_holder {if $aAd.type_id == 1}only-img{else}only-html{/if}">
	            ...
	            {if $aAd.type_id == 1}
		            <div class="bts-block__img">
			            <!-- Image type -->
			            <a {if !empty($aAd.url_link)}href="{$aAd.url_link}" target="_blank"{else}role="button"{/if} class="js_hover_title bts-block__thumb--img">
	            ...
	            {else}
		            ...
		            <div class="bts-block__html ad_unit_multi_ad">
                        {if !empty($aAd.image_path)}
			                <a {if !empty($aAd.trimmed_url)}href="{$aAd.trimmed_url}" target="_blank"{else}role="button"{/if} class="bts-block__thumb--img ad_unit_multi_ad_image">
			                	{img file=$aAd.image_path path='ad.url_image' server_id=$aAd.server_id}
				                <span class="js_hover_info">{$aAd.image_tooltip_text}</span>
			                </a>
                        {/if}
		                <div class="bts-block__info ad_unit_multi_ad_content">
			                <a href="{$aAd.trimmed_url}" class="bts-block__title ad_unit_multi_ad_title" target="_blank">{$aAd.title}</a>
			                <a class="ad_unit_multi_ad_url bts-block__url" href="{$aAd.trimmed_url}" target="_blank">{$aAd.trimmed_url}</a>
