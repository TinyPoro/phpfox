...
class Mail extends Phpfox_Service
{
    ...
    public function getConversationForAdmin($aConds, $iPage, $iSize)
    {
        ...
        if($iCnt)
        {
            ...
            foreach($aRows as $iKey => $aRow)
            {
                if($aRow['is_group'])
                {
                    $sTitle = $this->getGroupTitle($aRow['thread_id']);
                    $aRows[$iKey]['thread_name'] = !empty($sTitle) ?  $sTitle : $this->getDefaultGroupTitle($aRow['thread_id']);
                }
                else
                {
                    $aUsers = db()->select('u.full_name')
                        ->from(Phpfox::getT('mail_thread_user'), 'mtu')
                        ->join(Phpfox::getT('user'), 'u', 'u.user_id = mtu.user_id')
                        ->where('mtu.thread_id = '.$aRow['thread_id'])
                        ->execute('getSlaveRows');
                    $sTitle = '';
                    foreach($aUsers as $aUser)
                    {
                        $sTitle.=$aUser['full_name'].' & ';
                    }
                    $aRows[$iKey]['thread_name'] = trim($sTitle, '& ');
                }
            }
        }
        return [$iCnt,$aRows];
     ...
     * @return string
     */
    public function getDefaultGroupTitle($iThreadId)
    {
        ...
        {
            $sThreadName =  rtrim(rtrim($sThreadName), ',')._p(' and you');
        }
        return $sThreadName;
    ...
    public function getUnseenTotal()
    {
        $iCnt = (int) $this->database()->select('COUNT(*)')
            ...
            ->where('m.user_id = ' . Phpfox::getUserId() . ' AND m.is_read = 0 AND mt.last_id > 0 AND (m.last_message_id_for_delete < mt.last_id)')
            ->execute('getSlaveField');
        ...
        if($iLastMessage > $iLastMessageIdIfDelete)
        {
            ...
            foreach ($aMessages as $iKey => $aMail)
            {
                ...
                if($iCheckUserId == 0 && $iCount == 0 && empty($sDate))
                {
                    ...
                    $sDate = date('m/d/Y',$aMail['time_stamp']);

                }
                elseif (((int)$iCheckUserId == (int)$aMail['user_id']) && ($sDate == date('m/d/Y', $aMail['time_stamp'])) && $iCount > 0)
                {
                    if($iCount == 1)
                    {
                ...
                $bIsOnlyImageText = false;
                $bIsHasImageAndText = false;
