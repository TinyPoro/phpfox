    ...
    public function onDeleteUser($iUser)
    {
        ...
        foreach ($aThreads as $aThread) {
            ...
            if ($iCount > 2) {
                ...
                $this->database()->update(Phpfox::getT('mail_thread'), array('last_id' => !empty($aLastMess) ? $aLastMess['message_id'] : 0, 'time_stamp' => !empty($aLastMess) ? $aLastMess['time_stamp'] : 0), 'thread_id = ' . $aThread['thread_id']);
                if(!empty($aLastMess))
                {
                ...
                $this->database()->update(Phpfox::getT('mail_thread'), array('last_id_for_admin' => !empty($aLastMessForAdmin) ? $aLastMessForAdmin['message_id'] : 0, 'time_stamp' => !empty($aLastMessForAdmin) ? $aLastMessForAdmin['time_stamp'] : 0), 'thread_id = ' . $aThread['thread_id']);
