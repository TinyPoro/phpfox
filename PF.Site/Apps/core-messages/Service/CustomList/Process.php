...
class Process extends Phpfox_Service
{
    ...
    public function addMessageForCustomList($aVals, $bIsPageClaim = false)
    {
        ...
        if (empty($aCustom))
        {
            return Phpfox_Error::set(_p('Invalid Custom List'));
        }
        ...
        foreach($aUsers as $iUserId)
        {
            ...
            else
            {
                ...
                foreach ($aTemp as $iUserId)
                {
                    ...
                    Phpfox::getLib('mail')->to($iUserId)
                        ->subject(array('mail.full_name_sent_you_a_message_on_site_title', array('full_name' => Phpfox::getUserBy('full_name'), 'site_title' => Phpfox::getParam('core.site_title')), false, null, $aUser['language_id']))
                        ->message(array('mail.full_name_sent_you_a_message_no_subject', array(
                                'full_name' => Phpfox::getUserBy('full_name'),
                                'message' => $oFilter->clean(strip_tags(Phpfox::getLib('parse.bbcode')->cleanCode(str_replace(array('&lt;', '&gt;'), array('<', '>'), $aVals['message'])))),
                                'link' => $sLink
            ...
            $this->database()->update(Phpfox::getT('mail_thread'), array('last_id_for_admin' => (int) $iTextId), 'thread_id = ' . (int) $iId);
    ...
    public function changeCustomListTitle($aVals)
    {
        ...
        if(empty($aRow))
        {
            return Phpfox_Error::set(_p('The custom list not exists'));
        }
