...
class SendMessageController extends Phpfox_Component
{
    public function process()
    {
        ...
        if (($iNewId = Phpfox::getService('mail.process')->add($aVals)) && $bPass)
        {
            ...
            {
                $aDate = Phpfox::getService('mail.helper')->processDate($aMessages[1]['time_stamp']);
                if($aDate[1] != 'Today')
                {
                    $aMessages[0]['message_time_list'][0] = date('m_d_Y');
                    $aMessages[0]['message_time_list'][1] = 'Today';
                }
            }
            if(empty($aMessages[1]))
            {
            ...
            list($iCnt, $aMail, $aInputs) = Phpfox::getService('mail')->getSearch([], 0, 1);
            Phpfox_Template::instance()->assign(array(
                    'aMail' => $aMail[0],
                    'bIsSentbox' => false,
                    'bIsTrash' => false,
