...
class Ajax extends Phpfox_Ajax
{
    ...
    public function markConversationUnarchive()
    {
        ...
        return Phpfox_Error::set('mail_can_do_this_action');
    }
    ...
    public function deleteAttachmentText()
    {
        ...
        if(!empty($aMessage))
        {
            ...
            if(!empty($sNewText) || (int)$aMessage['total_attachment'] > 1)
            {
                ...
                {
                    $sUrl =  Phpfox_Url::instance()->makeUrl('attachment', array('download', 'id' => $iAttachmentId));
    ...
    public function deleteCustom()
    {
        ...
        {
            Phpfox_Error::set(_p('Can not delete. Something is wrong.'));
        }
    }
    ...
    public function loadEditCustomList()
    {
        ...
        $sContent = $this->getContent(false);
        $sTitleContentDefault = '<div class="js_custom_list_title custon-list-input" data-id="'.$aCustom['folder_id'].'"><span id="back-to-list-js" class="back-to-list hidden"><i class="ico ico-arrow-left-circle-o" aria-hidden="true"></i></span><span class="js_customlist_name_'.$aCustom['folder_id'].' fw-bold">'.$aCustom['name'].'</span><input type="text" class="js_custom_list_title_change" value="'.$aCustom['name'].'" style="display:none;"></div>';
        $sTitleContentDefault .='<div class="dropdown remove-list-group"><span class="btn fz-16" data-toggle="dropdown"><i class="ico ico-gear-o" aria-hidden="true"></i></span><ul class="dropdown-menu dropdown-menu-right"><li class="item_delete"><a href="'.\Phpfox_Url::instance()->makeUrl('mail.customlist',['delete' => $aCustom['folder_id']]).'" class="sJsConfirm" data-message="'._p('are_you_sure').'"><i class="fa fa-trash-o" aria-hidden="true"></i>'._p('Delete').'</a></li><li><a href="'.\Phpfox_Url::instance()->makeUrl('mail',['customlist_id' => $aCustom['folder_id']]).'"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>'._p('Send Messages').'</a></li></ul></div>';
        ...
        {
            $aUserIds = array_column($aCustom['users'],'user_id');
            $sUser = implode(',', $aUserIds);
            $this->call('aCustomlistMembers = ['.$sUser.'];');
    ...
    public function loadAddCustomList()
    {
        ...
        $sContent = $this->getContent(false);
        $sTitleContentDefault = '<div class="fw-bold create-custom"><span id="back-to-list-js" class="back-to-list hidden"><i class="ico ico-arrow-left-circle-o"></i></span>'._p('Create New Custom List').'</div>';
        $this->html('.js_core_messages_custom_list #js_content', $sContent);
        $this->html('.js_core_messages_custom_list #js_title_content', $sTitleContentDefault);
        $this->call('$Core.loadInit();');
        $this->call('coreMessages.customListAction();');
    ...
    public function changeGroupTitle()
    {
        Phpfox::isUser(true);
        $aVals = $this->get('val');
        list($bSuccess, $sNewTitle) = Phpfox::getService('mail.process')->changeGroupTitle($aVals);
        if (Phpfox_Error::isPassed())
        {
            $this->call('tb_remove();');
    ...
    public function composeProcess()
    {
        ...
        if (empty($aVal['to']) && empty($aVal['customlist'])) {
            ...
            $this->call('$(\'#\' + tb_get_active()).find(\'.js_box_content:first #js_ajax_compose_error_message\').html(\'<div class="error_message">' . str_replace("'", "\\'", _p('Please select at least one friend or one custom list')) . '</div>\');');
            return false;
        }
