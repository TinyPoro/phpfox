...
var coreMessages = {
            ...
            callbacks: {
                onScroll: function () {
                    if (this.mcs.topPct == 0) {
                        ...
                        console.log('Can continue to load more: ' + coreMessages.bContinueLoadMoreConversationContent);
                        console.log('Can continue to load more for next times: ' + Boolean(canLoadMore));
    ...
    composeMailThread: function () {
        $('.js_ajax_compose_message').off('submit').submit(function () {
            ...
            if (iCount > iLimit) {
                $('#js_ajax_compose_error_message').html('<div class="error_message ">Number of members for group you creating is over the limitation. The limitation number is ' + iLimit + '</div>');
            } else {
    ...
    initCustomListItemAction: function () {
        $('.js_core_messages_custom_list .js_customlist_item_click').click(function () {
            var iId = $(this).data('id');
    ...
    customListAction: function () {
        $('#js_btn_submit').click(function () {
            ...
            if (iCheckName == 0 && iCheckEdit == 0) {
                sError += '<div class="error_message ">Invalid Name</div>';
            }
            if (iCheckUsers == 0) {
                sError += '<div class="error_message ">Friend list can not null</div>';
            }

            if (iCheckUsers > iMemberCustomListMaximum) {
                sError += '<div class="error_message ">Number of members is over the limitation. The limitation is ' + iMemberCustomListMaximum + '</div>';
                if(iCheckEdit)
                {
            ...
            if (sError.length > 0) {
                ...
                if (iCheckEdit == 0 && (iCurrentCustomListUser + 1 > iCustomListMaximum)) {
                    sError = '<div class="error_message ">You can not create more custom lists because of limitation. The limitation is ' + iCustomListMaximum + '</div>';
                    $('.js_core_messages_custom_list #js_content .js_core_messages_add_customlist .error-list').html(sError);
                } else {
                    form.trigger('submit');
                }
        ...
        $('#js_check_all_custom_list').click(function () {
        ...
        $('.js_custom_list_mass_action').click(function () {
    ...
    initMembersCustomList: function () {
        var container = $('#js_friend_search_content');
        container.imagesLoaded(function () {
            if (!empty(aCustomlistMembers) && Array.isArray(aCustomlistMembers)) {
                ...
                $(aCustomlistMembers).each(function (index, value) {
                    if (!coreMessages.aSelectedCustomlistUsers[value]) {
                        ...
                        coreMessages.aSelectedCustomlistUsers[value] = 1;
                    }
                });
            }
       });
...
var coreMessagesCustomConversationMassActions = {
    ...
    initCheckAll: function () {
        $('#js_select_all_conversation').click(function () {
    ...
    initAction: function () {
        $('.js_conversation_mass_actions').off('click').click(function (e) {
            ...
            $('.js_conversation_item_check:checked').each(function (index, value) {
                $('<input type="hidden" name="conversation_action[]" class="js_conversation_item_check"/>').
                val($(value).val()).
                appendTo(holder);
            })
            var action = $(this).data('action');
...
var coreMessagesCustomAttachment = {
    ...
    deleteAttachment: function () {
        $('#js_compose_new_message .attachment-row:not(".rebuilt")').each(function () {
            ...
            $(this).find('.js_attachment_remove_inline').find('a:first').click(function(){
    ...
    removeInlineWithDelete: function(iAttachmentId) {
        $Core.jsConfirm({},function(){
                ...
                setTimeout(function(){coreMessagesCustomAttachment.checkAttachment();},100);
            },
        function(){});
        return false;
    },
