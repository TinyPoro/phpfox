...
class Callback extends \Phpfox_Service
{
    ...
    public function getActivityFeed($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        $aReturn = array(
            ...
            'no_user_show' => true,
            'feed_content' => ($aRow['total_like'] == '1' ? _p('1 member') : _p('total_members', array('total' => $aRow['total_like']))),
    ...
    public function getCommentNotificationTag($aNotification)
    {
        ...
        $sPhrase = _p('full_name_tagged_you_on_a_group', array('full_name' => Phpfox::getService('notification')->getUsers($aRow)));
    ...
    public function deleteFeedItem($iItemId)
    {
        ...
        db()->delete(':feed', ['type_id' => 'groups_comment', 'item_id' => $iItemId]);
    ...
    public function addLike($iItemId, $bDoNotSendEmail = false, $iUserId = null)
    {
        ...
        if ($iUserId === null) {
            ...
            \Phpfox::getLib('mail')->to($iUserId)
                ->subject(array('Membership accepted to "{{ title }}"', array('title' => $aRow['title'])))
                ->message(array('Your membership to the group "<a href="{{ link }}">{{ title }}</a>" has been accepted. To view this group follow the link below: <a href="{{ link }}">{{ link }}</a>',
                    array('link' => $sLink, 'title' => $aRow['title'])))
                ->sendToSelf(true)
                ->send();
    ...
    public function getNotificationLike($aNotification)
    {
        ...
        if ($aNotification['user_id'] == $aRow['user_id']) {
            ...
            $sPhrase = _p('user_joined_your_group_title', array('users' => $sUsers, 'title' => $sTitle));
        } else {
    ...
    {
        $aRow = $this->database()->select('fc.feed_comment_id, fc.content, fc.user_id, e.page_id, e.title, u.')
            ->from(Phpfox::getT('pages_feed_comment'), 'fc')
            ->join(Phpfox::getT('pages'), 'e', 'e.page_id = fc.parent_user_id')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = fc.user_id')
            ->leftJoin(Phpfox::getT('pages_url'), 'pu', 'pu.page_id = e.page_id')
    ...
    public function getNotificationPosted_Video($aNotification)
    {
        ...
        $sPhrase = _p('full_name_posted_a_video_on_group_title',
            ['full_name' => Phpfox::getService('notification')->getUsers($aNotification), 'title' => $sGroupTitle]);
        ...
        if (empty($aPage)) {
            return array(
                'error' => _p('sorry_the_group_you_are_looking_for_no_longer_exists',
                    array('link' => Phpfox::getLib('url')->makeUrl('groups')))
            );
        }

        $aPage['link'] = Phpfox::permalink('groups', $aPage['item_id'], $aPage['title']);
        $aPage['paypal_msg'] = _p('sponsor_paypal_message_group', array('sGroupTitle' => $aPage['title']));
        $aPage['image_dir'] = 'pages.url_image';
        $aPage['title'] = _p('sponsor_title_group', array('sGroupTitle' => $aPage['title']));
    ...
    public function getNotificationReassign_Owner($aNotification)
    {
        ...
        $sPhrase = _p('full_name_just_assigned_you_as_owner_of_group_title', array(
            'full_name' => Phpfox::getService('notification')->getUsers($aNotification),
            'title' => \Phpfox::getLib('parse.output')->shorten($aRow['title'],
                \Phpfox::getParam('notification.total_notification_title_length'), '...')
        ));
    ...
    public function getNotificationOwner_Changed($aNotification)
    {
        ...
        $sPhrase = _p('full_name_just_transfer_your_group_title_to_other_user', array(
            'full_name' => Phpfox::getService('notification')->getUsers($aNotification),
            'title' => \Phpfox::getLib('parse.output')->shorten($aRow['title'],
                \Phpfox::getParam('notification.total_notification_title_length'), '...')
        ));
