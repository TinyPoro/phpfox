...
class Package extends Phpfox_Service
{
    ...
    {
        $aPackages = get_from_cache('activitypoint_packages', function(){
            $aRows = db()->select('*')
                ->from($this->_sTable)
                ->order('points ASC')
                ->where('is_active = 1')
                ->execute('getSlaveRows');
            $sDefautCurrency = Phpfox::getService('core.currency')->getDefault();
            foreach($aRows as $iKey => $aRow)
            {
                $aPrice = unserialize($aRow['price']);
                $aRows[$iKey]['default_price'] = [
                    'price' => $aPrice[$sDefautCurrency],
                    'symbol' => Phpfox::getService('core.currency')->getSymbol($sDefautCurrency),
                ];
            }
            return $aRows;
        });
