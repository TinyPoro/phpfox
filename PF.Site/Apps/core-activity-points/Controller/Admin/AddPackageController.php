...
class AddPackageController extends Phpfox_Component
{
    public function process()
    {
        ...
        if($aVals = $this->request()->getArray('val'))
        {
            ...
            {
                if(empty($aVals['points']) || (!empty($aVals['points']) && !is_numeric($aVals['points'])) || (!empty($aVals['points']) && is_numeric($aVals['points']) && (intval($aVals['points']) != $aVals['points'])) || (!empty($aVals['points']) && is_numeric($aVals['points']) && (intval($aVals['points']) == $aVals['points']) && (intval($aVals['points']) < 1)))
                {
        ...
        $this->template()->setTitle(($bIsEdit) ? _p('Edit Package') : _p('Create new package'))
                        ->assign([
