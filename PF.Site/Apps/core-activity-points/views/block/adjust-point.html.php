    ...
    {if !$bDisableAll && Phpfox::getUserParam('activitypoint.can_admin_adjust_activity_points')}
        ...
        <div class="form-group">
            ...
            <p class="js_maximum_points">
                {if !empty($iSentMaximumPoints) && !empty($iPointsCanSent)}
                <span class="js_point_title">{_p var='activitypoint_notify_maximum_point_for_send'}</span> <span id="point-number">{$iPointsCanSent}.</span>
                {elseif !empty($sMaximumPointsForReduce)}
                <span class="js_point_title">{_p var='activitypoint_notify_maximum_point_for_reduce'}</span> <span id="point-number">{$sMaximumPointsForReduce}.</span>
                {/if}
            </p>
        </div>
        ...
        <div class="form-group action-button">
            ...
            <button class="btn btn-default" onclick="js_box_remove(this);">{_p var='Cancel'}</button>
        </div>
    {else}
