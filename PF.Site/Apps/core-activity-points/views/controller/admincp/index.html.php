                    ...
                    <table class="table" cellpadding="0" cellspacing="0" style="margin-bottom: 0;">
                        ...
                        {foreach from=$aModules key=module_id item=aModule}
                            <tr class="js_admincp_table_header_toggle core-table-header-toggle {if $aModule.active}open{/if}" data-module="{$module_id}" data-togglecontent="{$module_id}" data-action="{if $aModule.active}close{else}open{/if}">
                                ...
                                <td class="text-center"></td>
                                <td class="text-center"></td>
                                <td class="text-center"><input type="checkbox" name="val[{$module_id}][is_active]" class="js_check_all_module" data-module="{$module_id}"  {if $aModule.bActive}checked{/if}></td>
                            </tr>
                            ...
                            <tr class="js_toggle_module_{$module_id} {if $aModule.active}open{/if} core-table-content-toggle">
                                ...
                                <td class="text-center js_point_setting w160"><span class="js_value_text">{$aSetting.value}</span><input type="hidden" value="{$aSetting.value}" name="val[{$module_id}][settings][{$var_name}][value]" class="js_change_value form-control"></td>
                                <td class="text-center js_point_setting w160"><span class="js_value_text">{$aSetting.max_earned}</span><input type="hidden" value="{$aSetting.max_earned}" name="val[{$module_id}][settings][{$var_name}][max_earned]" class="js_change_value form-control"></td>
                                <td class="text-center js_point_setting w160"><span class="js_value_text">{$aSetting.period}</span><input type="hidden" value="{$aSetting.period}" name="val[{$module_id}][settings][{$var_name}][period]" class="js_change_value form-control"></td>
                                <td class="text-center"><input type="checkbox" name="val[{$module_id}][settings][{$var_name}][is_active]" class="js_check_setting_{$aSetting.module_id}" {if $aSetting.is_active}checked{/if} {if !$aModule.bActive}disabled{/if}></td>
                            </tr>
                            {/foreach}
                        {/foreach}
