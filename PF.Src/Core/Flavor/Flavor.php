    ...
    public function save($type, $values, $sub_type = '')
    {
        ...
        switch ($type) {
            ...
            case 'design':
                ...
                if (!file_exists($less_file)) {
                    ...
                    foreach ($lines as $line) {
                        ...
                        if (substr($line, 0, 1) == '@') {
                            ...
                            if(in_array($var, $brandVariables) && isset($values[$var]) && $value != $values[$var]) {
                                $hasCustomBrand = true;
                            }
                ...
                if($hasCustomBrand) {
                    $less_file = $this->path . 'less' . PHPFOX_DS . 'variables.less';
                    if(file_exists($less_file)) {
                        $less = '';
                        $lines = file($less_file);
                        foreach ($lines as $line) {
                    ...
                    case "skin":
                        ...
                        foreach ($value->options as $option) {
                            $html .= strtr('<li><a class="edit_for_theme :selected" href="#" data-url=":url" value=":key" label=":label" ><span class="item-square-skin-color" style="background: :primary;"></span>:label</a></li>', [
                                ...
                                ':url' => url('/flavors/manage', ['id' => $this->id, 'type' => 'design', 'theme_suffix' => (($cur_skin == $option->value)?'':$option->value)]),
                            ]);
                        }
                        $html .= '</ul></div>';
