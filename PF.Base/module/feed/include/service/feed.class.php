...
class Feed_Service_Feed extends Phpfox_Service
{
    ...
    {
        if ($aFeed['type_id'] != 'feed_comment' && $aFeed['type_id'] != 'feed_egift') {
            return [];
        }
        $aUsers = $this->database()->select('td.*, '. \Phpfox::getUserField())
            ->from(':feed_tag_data','td')
            ->join(':user','u','u.user_id = td.user_id')
            ->where('td.item_id = '.(int)$aFeed['item_id'].' AND td.type_id = \'feed_comment\'')
            ->execute('getSlaveRows');
    ...
    public function checkTaggedUser($iItemId, $sType, $iUserId)
    {
        return db()->select('user_id')
            ->from(':feed_tag_data')
            ->where('item_id = '.(int)$iItemId.' AND type_id = \''.$sType.'\' AND user_id = '. (int)$iUserId)
            ->execute('getSlaveRow');
    }
}