...
class Feed_Service_Callback extends Phpfox_Service
{
	...
	{		
        ...
        $aSubject = (Phpfox::getUserId() == $iSenderId ?
            array('full_name_commented_on_one_of_gender_wall_comments',
                array('full_name' => Phpfox::getUserBy('full_name'),
                    'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1)
            ))
            :
            array('full_name_commented_on_one_of_row_full_name_s_wall_comments',
                array('full_name' => Phpfox::getUserBy('full_name'),
                    'row_full_name' => $aRow['full_name'])
            ));

        $aMessage = (Phpfox::getUserId() == $iSenderId ?
            array('full_name_commented_on_one_of_gender_wall_comments_message',
                array('full_name' => Phpfox::getUserBy('full_name'),
                    'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1), 'link' => $sLink))
            :
            array('full_name_commented_on_one_of_row_full_name_s_wall_comments_message',
                array('full_name' => Phpfox::getUserBy('full_name'),
        ...
        Phpfox::getService('comment.process')->notify(array(
				...
				'item_id' => $aRow['feed_comment_id'],
				'owner_subject' => ['full_name_commented_on_one_of_your_wall_comments', array('full_name' => Phpfox::getUserBy('full_name'))],
				'owner_message' => ['full_name_commented_on_one_of_your_wall_comments_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a', array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink)],
	...
	public function getCommentNotificationFeed($aNotification)
	{
		...
		elseif ($aRow['user_id'] == Phpfox::getUserId())		
		{
			...
			{
				$sPhrase = _p('parent_user_name_commented_on_one_of_your_status_updates', array('parent_user_name' => '<span class="drop_data_user">' . Phpfox::getLib('parse.output')->shorten($aRow['parent_full_name'], 0) . '</span>'));
			}
		}
		else 
		{
    ...
    public function getNotificationTagged_Profile($aNotification)
    {
        ...
        $sPhrase = _p('user_name_tagged_you_in_a_status_update', array('user_name' => Phpfox::getService('notification')->getUsers($aNotification)));
