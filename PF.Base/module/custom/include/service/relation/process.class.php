...
class Custom_Service_Relation_Process extends Phpfox_Service
{
    ...
    public function updateRelationship($iRelationType, $iWith = null, $iUser = null, $iRelationDataId = null)
    {
        ...
        if($iRelationDataId) {
            $this->cache()->remove(array('relations', "user_" . $iUser . "_relation_" . $iRelationDataId));
            $this->cache()->remove(array('relations', "user_" . $iWith . "_relation_" . $iRelationDataId));
        }
    ...
    public function denyStatus($iStatusId, $iUser = null)
    {
        ...
        $this->cache()->remove(array('relations', "user_" . $aRequest['user_id'] . "_relation_" . $iStatusId));
        $this->cache()->remove(array('relations', "user_" . $aRequest['with_user_id'] . "_relation_" . $iStatusId));

        $this->cache()->remove(array('reluser', $aRequest['user_id']));
        $this->cache()->remove(array('reluser', $aRequest['with_user_id']));
