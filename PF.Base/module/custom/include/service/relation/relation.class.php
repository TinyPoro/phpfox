...
class Custom_Service_Relation_Relation extends Phpfox_Service
{
    ...
    public function getLatestForUser($iUserId, $iBeforeId = null, $bStarted = false)
    {
        ...
        if (false !== ($aRelation = $this->cache()->get($sCacheId))) {
            return $aRelation;
        }
    ...
    {
        $row = $this->database()->select('*')->from(':custom_relation')->where(['relation_id' => $id])->execute('getRow');
        return (isset($row['phrase_var_name'])?_p($row['phrase_var_name']):'');
    }
}
