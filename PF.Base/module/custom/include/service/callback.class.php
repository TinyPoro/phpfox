...
class Custom_Service_Callback extends Phpfox_Service
{
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => array('full_name_commented_on_your_profile_update_title', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'title' => $this->preParse()->clean(_p($aRow['phrase_var_name']), 100)
                )),
                'owner_message' => array('full_name_commented_on_your_profile_update_message', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'link' => $sLink,
                ...
                'mass_id' => 'custom',
                'mass_subject' => (Phpfox::getUserId() == $aRow['user_id'] ? array('full_name_commented_on_gender_profile_update',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1)
                    )) : array('full_name_commented_on_row_full_name_s_video',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'row_full_name' => $aRow['full_name']))),
                'mass_message' => (Phpfox::getUserId() == $aRow['user_id'] ? array('full_name_commented_on_gender_profile_update_message',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1),
                        'link' => $sLink,
                        'title' => array($aRow['phrase_var_name'])
                    )) : array('full_name_commented_on_row_full_name_s_profile_update', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
