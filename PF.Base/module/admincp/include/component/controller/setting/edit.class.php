    ...
    public function process()
    {
        ...
        foreach ($aSettings as $index => $aSetting) {
            ...
            if (isset($aCookieValue[$aSetting['var_name']])) {
                if (in_array($aSetting['var_name'], ['method', 'mail_smtp_secure'])) {
                    if (is_array($aCookieValue[$aSetting['var_name']]['real'])) {
                        $aCookieValue[$aSetting['var_name']]['real'] = implode(',', $aCookieValue[$aSetting['var_name']]['real']);
                    }
                    $aSettings[$index]['value_actual'] = $aCookieValue[$aSetting['var_name']]['real'];
                } else {
                    if (is_array($aCookieValue[$aSetting['var_name']])) {
                        $aCookieValue[$aSetting['var_name']]['real'] = implode(',', $aCookieValue[$aSetting['var_name']]);
                    }
                    $aSettings[$index]['value_actual'] = $aCookieValue[$aSetting['var_name']];
                }
            }
            $aSettings[$index]['is_file_config'] = Phpfox::getLib('setting')->getFromServerConfigFile($aSetting['module_id'] . '.' . $aSetting['var_name']);
        }
