...
class Comment_Component_Ajax_Ajax extends Phpfox_Ajax
{
    public function add()
    {
        ...
        if(empty($aVals['parent_id'])) {
            if (in_array($aVals['type'], ['feed', 'pages', 'groups'])) {
                $aVals['type'] = $aVals['type'] . '_comment';
            }
            $this->call('if (typeof $Core.updateCommentCounter != \'undefined\') {$Core.updateCommentCounter(\'' . $aVals['type'] . '\', ' . $aVals['item_id'] . ', \'+\');}');
        }
    ...
    public function inlineDelete()
    {
        ...
        if (Phpfox::getService('comment.process')->deleteInline($this->get('comment_id'), $sTypeId)) {
            ...
            if ($sTypeId && $iItemId = $this->get('item_id')) {
                if (in_array($sTypeId, ['feed', 'pages', 'groups'])) {
                    $sTypeId .= '_comment';
                }
                $this->call('if (typeof $Core.updateCommentCounter != \'undefined\') {$Core.updateCommentCounter(\'' . $sTypeId . '\', ' . $iItemId . ', \'-\');}');
            }
        }
    }
