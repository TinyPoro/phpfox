...
<div class="profiles_banner {if isset($aCoverPhoto.server_id)}has_cover{/if}" {if $sCoverDefaultUrl}style="background-image:url({$sCoverDefaultUrl})"{/if}>
	...
	<div class="profile_viewer_actions dropdown">
		...
		{if Phpfox::isAppActive('Core_Messages') && Phpfox::getService('user.privacy')->hasAccess('' . $aUser.user_id . '', 'mail.send_message') && (!Phpfox::getUserParam('mail.restrict_message_to_friends') || (Phpfox::getUserParam('mail.restrict_message_to_friends') && isset($aUser.is_friend) && $aUser.is_friend === true))}
