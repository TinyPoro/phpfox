...
class Log_Service_Session extends Phpfox_Service
{
	...
	public function setUserSession()
	{
        ...
        {
            if ((isset($this->_aSession['ls_last_activity']) && $this->_aSession['ls_last_activity'] < Phpfox::getService('log.session')->getActiveTime())
		...
		if (Phpfox::isUser()) {
            if (!Phpfox::getParam('user.disable_store_last_user') && (Phpfox::getUserBy('last_activity') < Phpfox::getService('log.session')->getActiveTime() || (isset($this->_aSession['ls_user_id']) && $this->_aSession['ls_user_id'] == 0))) {
	...
	public function getOnlineStats()
	{
		$sOnlineMembers = $this->database()->select('COUNT(DISTINCT user_id)')
			...
			->where('user_id > 0 AND last_activity > ' . Phpfox::getService('log.session')->getActiveTime())
