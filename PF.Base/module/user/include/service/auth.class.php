...
class User_Service_Auth extends Phpfox_Service
{
	...
	public function __construct()
	{
		...
		{	
			if ($iUserId > 0)
			{
                ...
                if (Phpfox::isAppActive('Core_BetterAds') && Phpfox::getParam('ad.advanced_ad_filters')) {
                    $bLoadUserField = true;
	...
	public function login($sLogin, $sPassword, $bRemember = false, $sType = 'email', $bNoPasswordCheck = false)
	{
		...
		if (!$oBan->check('email', $aRow['email'])
            || !$oBan->check('username', $aRow['user_name'])
            || !$oBan->check('display_name', $aRow['full_name']))
		{
	...
	public function logout()
	{
		...
		if (!empty($this->_aUser['user_id']))
		{
            ...
            $this->database()->delete(Phpfox::getT('log_session'), array('user_id' => $this->_aUser['user_id']));
		}
