...
class User_Service_Group_Setting_Setting extends Phpfox_Service
{
    ...
    public function getForSearch($aSkipModules = [])
    {
        ...
        foreach ($aSettings as $aRow) {
            $aRows[] = [
                ...
                'title'     => strip_tags(htmlspecialchars_decode((Core\Lib::phrase()->isPhrase('admincp.user_setting_' . $aRow['name'])
    ...
    {
        // The codes below no remove, because it support restful authenticate (use for force user or set user logged in)
        ...
        if (isset($this->_aParam[$sName])) {
            /**
             * Optimize: Fix bug unserialize only used for string only
             * If not string, unserialize always return false
             */
            if (is_string($this->_aParam[$sName])) {
                $aUnserialize = @unserialize($this->_aParam[$sName]);
            }
            else {
                $aUnserialize = $this->_aParam[$sName];
            }
    ...
    public function get($iGroupId, $iModuleId = null)
    {
        ...
        foreach ($aRows as $aRow) {
            ...
            if(!Phpfox::isAppActive('Core_Activity_Points') && preg_match('/points_/',$aRow['name'])) {
                continue;
            }
            $aParts = explode('</title><info>',
                Phpfox::isPhrase('user_setting_' . $aRow['name'])
                    ? _p('user_setting_' . $aRow['name']) : $aRow['name']);
    ...
    public function getSetting($iId)
    {
        ...
        $aRow['info'] = (Core\Lib::phrase()->isPhrase('admincp.user_setting_' . $aRow['name'])
            ? _p('user_setting_' . $aRow['name']) : $aRow['name']);
    ...
    public function _setParam($iGroupId)
    {
        ...
        if (false === ($aParams = $this->cache()->getLocalFirst($sCacheId))) {
            ...
            $this->cache()->saveBoth($sCacheId, $aParams);
