...
class User_Service_Process extends Phpfox_Service
{
    ...
    public function updateStatus($aVals)
    {
        ...
        if ($bIsTagged)
        {
            if (empty($aInsert['location_latlng'])) {
                $this->notifyTagged($sStatus, $iStatusId, 'status', $aVals['privacy'], $aTagged);
            } else {
                $this->notifyTagged($sStatus, $iStatusId, 'status', $aVals['privacy'], $aTagged, [
            ...
            foreach ($aTagged as $iUserId) {
                ...
                Phpfox::getLib('mail')->to($iUserId)
                    ->subject(['user_name_tagged_you_in_a_status_update', ['user_name' => $sTagger]])
                    ->message(['user_name_tagged_you_in_a_status_update_check_it_out',
                                ['user_name' => $sTagger, 'link' => $link]])
                    ->send();
            }
        }

        $this->notifyTagged($sStatus, $iStatusId, 'status', $aVals['privacy']);
    ...
    public function editStatus($iFeedId, $aVals)
    {
        ...
        if ($bIsTagged && count($aTagged))
        {
            ...
            foreach ($aTagged as $iUserId) {
                // update feed comment on mention or tagged feed
                // -- privacy
                $this->database()->update(Phpfox::getT('feed'), ['privacy' => $aVals['privacy']], [
                    'time_stamp' => $aStatusFeed['time_stamp'],
                    'user_id'    => $aStatusFeed['user_id'],
                    'type_id'    => 'feed_comment',
                    'parent_user_id' => $iUserId]);
                // -- feed comment
                $this->database()->update(Phpfox::getT('feed_comment'), $aUpdate, [
                    'time_stamp' => $aStatusFeed['time_stamp'],
                    'user_id'    => $aStatusFeed['user_id'],
                    'parent_user_id' => $iUserId]);
    ...
    */
    ...
    public function notifyTagged($sContent, $iItemId, $sType, $iPrivacy = 0, $aMatches = null, $aLocation = [])
    {
        $bIsMention = false;
    ...
    public function processInactiveJob($iJobId)
    {
        ...
        $bSent = $oMail->subject('user.inactive_member_email_subject')
            ->message('user.inactive_member_email_body')
            ->send();
        if ($bSent) {
