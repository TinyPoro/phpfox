...
var friend_AddFriendsToList = {
	init: function(){
        $('.js_core_menu_add_friend_to_list').click(function(){
        ...
        $('#js_add_friend_to_list_btn').on('click', function(){
        	if($('.js_global_item_moderate').length)
			{
				var sUserIds = '';
                $('.js_global_item_moderate:checked').each(function(){
                	sUserIds += $(this).val() + ',';
				})
