			...
			foreach ($mentions as $user) {
                ...
                if ($bSendMail) {
                    Phpfox::getLib('mail')->to($user->id)
                        ->subject($sTagger . ' ' . _p('mentioned_you_in_a_post', [], $user->language_id))
                        ->message($sTagger . ' ' . _p('mentioned_you_in_a_post', [], $user->language_id) . ' <a href="' . $link . '">' . _p('check_it_out', [], $user->language_id) . '</a>')
                        ->translated()
                        ->send();
                }
			}
