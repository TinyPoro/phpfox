                            ...
                            case 'redis':
                                ...
                                if(extension_loaded('redis')) {
                                    $cache = CacheManager::getInstance('redis',[
                                        'host' => $cache_file['redis']['host'],
                                        'port' => $cache_file['redis']['port'],
                                        'securityKey' => self::unitCacheDomain()
                                    ]);
                                    // Init session handler
                                    if (!defined('PHPFOX_SKIP_INIT_SESSION')) {
                                        ini_set('session.save_handler', 'redis');
                                        ini_set('session.save_path', 'tcp://' . $cache_file['redis']['host'].':'.$cache_file['redis']['port']);
                                    }
                                }
                                else {
