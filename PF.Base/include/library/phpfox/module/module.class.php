...
class Phpfox_Module
{
        ...
        if ((!Phpfox::isAdminPanel() && !defined('PHPFOX_INSTALLER'))) {
            ...
            if ($meta === false) {
                $query = function ($name) use ($db){
                    return $db->select('*')->from(':theme_template')->where(['type_id' => 'meta', 'name' => $name])->execute('getRow');
                };
            ...
            if (isset($this->_aModuleBlocks[$sController][$iId])
                ...
                foreach ($aCachedBlocks as $sKey => $sValue) {
                    ...
                    if (isset($this->_aBlockWithSource[$sController][$iId][$sKey])
                        ...
                        if (($aCacheData = $oCache->getLocalFirst($sCacheId))
    ...
    private function _cacheModuleBlocks()
    {
        ...
        if ((false === ($this->_aModuleBlocks = $oCache->getLocalFirst($iBlockCacheId)))
            || (false ===($this->_aBlockLocations = $oCache->getLocalFirst($sLocationCacheId)))
            ...
            $oCache->saveBoth($sLocationCacheId, $this->_aBlockLocations);
